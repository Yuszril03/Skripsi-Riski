        ...
        document.getElementById('defaultKataSAndi').addEventListener('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const updateAccount = {};
                    updateAccount['/Data-Administrator/<?= session()->get('Id') ?>'] = DataAccount;
                    update(ref(db), updateAccount);

                    $.ajax({
                        url: "<?= base_url() ?>/Default-Kata-Sandi-Admin/" + md5('12345678'),
                        dataType: "JSON",
                        type: "GET"
                    }).done((result) => {
                        Swal.fire({
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (jumlah == 0) {
                        ...
                        let DataAccount = {
                            Email: `<?= session()->get('Email') ?>`,
                            KataSandi: md5(document.getElementById('newPas').value),
                            NamaAdmin: `<?= session()->get('Nama') ?>`
                        }
                        const updateAccount = {};
                        updateAccount['/Data-Administrator/<?= session()->get('Id') ?>'] = DataAccount;
                        ...
                        $.ajax({
                            url: "<?= base_url() ?>/Ubah-Kata-Sandi-Admin/" + md5(document.getElementById('newPas').value),
                            dataType: "JSON",
                            type: "GET"
                        }).done((result) => {
                            Swal.fire({
