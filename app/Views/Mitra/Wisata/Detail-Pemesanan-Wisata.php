...
<body class="hold-transition sidebar-mini layout-fixed">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">
                                <button class="btn" onclick="location.href=`<?= base_url() ?>/Data-Pemesanan-Tiket`" title="Kembali"><i class="fa fa-angle-left fa-2x"></i></button>
    ...
    <script type="module">
        ...
        const ValueItem11 = ref(db, 'Transaction-Wisata/<?= $DataID ?>');
        onValue(ValueItem11, (kontenns) => {
            let PostD = {

                BuktiTraksaksi: kontenns.val().BuktiTraksaksi,
                HargaAnak: kontenns.val().HargaAnak,
                HargaDewasa: kontenns.val().HargaDewasa,
                IdCutomer: kontenns.val().IdCutomer,
                IdMitra: kontenns.val().IdMitra,
                JenisPembayaran: kontenns.val().JenisPembayaran,
                JumlahAnak: kontenns.val().JumlahAnak,
                JumlahDewasa: kontenns.val().JumlahDewasa,
                Rating: kontenns.val().Rating,
                StatusTransaksi: kontenns.val().StatusTransaksi,
                TotalAnak: kontenns.val().TotalAnak,
                TotalDewasa: kontenns.val().TotalDewasa,
        ...
        document.getElementById('submitData').addEventListener('click', function() {
            ...
            if (angka == 0) {
                let PostD = {
                    ...
                    HargaAnak: parseJsonMitra.HargaAnak,
                    HargaDewasa: parseJsonMitra.HargaDewasa,
                    IdCutomer: parseJsonMitra.IdCutomer,
                    IdMitra: parseJsonMitra.IdMitra,
                    JenisPembayaran: parseJsonMitra.JenisPembayaran,
                    JumlahAnak: parseJsonMitra.JumlahAnak,
                    JumlahDewasa: parseJsonMitra.JumlahDewasa,
                    Rating: parseJsonMitra.Rating,
                    StatusTransaksi: parseJsonMitra.StatusTransaksi,
                    TotalAnak: parseJsonMitra.TotalAnak,
                    TotalDewasa: parseJsonMitra.TotalDewasa,
                ...
                const updates = {};
                updates['Transaction-Wisata/<?= $DataID ?>'] = PostD;
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        location.href = "<?= base_url() ?>/Detail-Pemesanan-Tiket"
                    }
                })
            } else {
                Swal.fire({
